 ...
 * - research(topic)：给主题（如「收集 2025 财年可口可乐与百事的全部财报与研报」），隔离写手
 *   子会话自主分解研究线 → web_search/web_fetch 定向查证 → 自检补漏 → 合成带来源完整报告
 *   → 收据回全文。零宿主参数（无 project/source/path/label）——留档是调用方自己的事；
...
function makeResearchTool(spec: { configWorkspace?: string; routeKey?: string; routes?: unknown; ctx: HostContext }): unknown {
  return dtool({
    name: "research",
    description:
      "通用主题式深度调研（deep-research 型，唯一入口）：给一个主题（如「收集 2025 财年可口可乐与百事的全部财报与研报」），隔离写手子会话自主分解研究线 → web_search/web_fetch 定向查证 → 自检补漏 → 合成带来源的完整调研报告（内联 [SRC-n] + 编号来源清单）交回调用方。注意：这是深度调研工具，不是轻量单查——单点小问题直接 web_search 即可，别占这里。搜索 provider 读宿主 web seam 配置（exa/deepseek，配谁用谁）。单路由口径：fallback 未启用。",
    parameters: {
    ...
    async execute(rawArgs, rawExec) {
      ...
      const out = await runResearch(
        { topic: args?.topic, routes: spec.routes, routeKey: spec.routeKey },
