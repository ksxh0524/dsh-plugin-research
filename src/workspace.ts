 ...
 *
 * 优先级：env AV_WORKSPACE > 自起点向上找中性锚 `.av/workspace.json`（kind=auto-cut）。
...
export function resolveWorkspaceRoot(startDir?: string, env?: Record<string, string | undefined>): string {
  const e = String((env ?? process.env).AV_WORKSPACE ?? "").trim();
  if (e) return resolve(e);
  let dir = resolve(startDir || process.cwd());
  const tried: string[] = [];
  for (;;) {
    const anchor = join(dir, ".av", "workspace.json");
    tried.push(anchor);
    try {
  ...
  throw new Error(`[工作区未找到] 出路：① 设 AV_WORKSPACE=<工作区根>；② 在工作区根放 .av/workspace.json。已试：\n${tried.join("\n")}`);
}
