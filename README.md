...
- **One shared config by default**: model (the box has a dropdown — every configured model is one click away, or type `provider/model` by hand; **empty = follow** — resolved live at call time against the calling session's workspace router table, so it differs per session and the card shows no fixed value) + thinking intensity (low/medium/high/xhigh; empty = follow the route declaration) — shared by the Researcher and Reviewer isolated stages;
