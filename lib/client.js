 ...
 *  DSH 实例 PluginConfigForm 同形折叠卡（索引仓 `docs/settings-cards.md` §4.1/§4.2：默认折叠成一行，点开才渲染控件；
 *  改输入只落草稿，保存是唯一写点；只读文档禁用控件并说明；落盘后才收起；真 label 关联）。
 *
 *  席位契约（DSH 实例 0.1.6，语义锚 = dsh-client-ui-plugin-manager bundle 的 plugins.item 槽 + 官方
 *  BashCard/AgentLoopCard 的 view 双态）：
 ...
 *
 *  形态规范 = 索引仓 `docs/settings-cards.md` §4.1/§4.2（照 DSH 实例 dsh-client-ui-settings-plugins 发布物 PluginConfigForm +
 *  fields.tsx + card-form.ts 的结构与状态语义，逐项对齐）：
 *  - **默认折叠成一行**：卡壳由卡自己出 <li>（边框/底色/圆角同 DSH 实例卡值）；
 *    折叠头是 <button aria-expanded>（名称压描述两行 + 未保存 Tag + chevron），body 只在展开时渲染。
 *  - **暂存草稿**：改输入只落草稿，**保存是唯一写点**（DSH 实例 card-form 理由：一次设置写是带 revision 的
 ...
 *  - 复用面：Tag / chevron 图标走 DSH 实例冻结模块种子表的 `@deepseek-ai/dsh-client-ui-primitives`（索引仓 `docs/design-tokens.md` §4.3）；
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			// 卡壳（DSH 实例 .card/.cardOpen/.header/.headText/.name/.description/.chevron/.body/.footer）
			...
			// footer（DSH 实例 .footer/.discard/.save/.failed）
			...
			// 动效自护：DSH 实例 ui-theme 无全局 reduced-motion 兜底（DSH 实例 web-styling 规则 22），加了 transition 就得自己守卫
		...
		/* ---------- 设置卡组件（DSH 实例 PluginConfigForm 同形） ---------- */
		function ResearchCard(props) {
			...
			/** 唯一写点：整份草稿一次 setConfig；成功以 DSH 实例回读为基线并收起，失败留草稿。 */
			var save = useCallback(
				async function () {
					...
					try {
						...
						setLoaded(next);
						setDraft(next);
						setOpen(false); // DSH 实例同款：确认落盘才收起
					} catch (e) {
			...
			/** Tag / chevron 优先复用 DSH 实例共享控件（索引仓 `docs/design-tokens.md` §4.3 种子表），缺席退 CSS 替身。 */
			function tag(text) {
			...
			return h(
				...
				open
					? h(
							...
							field("thinking", "默认思考强度", "", "留空 = DSH 实例默认档。", thinkingControl("thinking")),
							h(
								"div",
								{ className: "rsch-adv" },
