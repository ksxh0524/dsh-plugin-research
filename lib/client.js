...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CONTRIBUTION = {
			...
			descriptors: [
				descriptor("getConfig", "_hint", { file: "src/cordis.ts", line: 84, column: 9 }),
				descriptor("setConfig", "patch", { file: "src/cordis.ts", line: 91, column: 9 }),
				descriptor("listModels", "_hint", { file: "src/cordis.ts", line: 103, column: 9 }),
			],
		};
		...
		function ResearchCard(props) {
			...
			var modelHint = "留空 = 跟随：调用时按发起会话所在工作区的路由表实时定（会话不同值就不同，卡上显示不了）；下拉选一个 = 写死用它，也可手填。";
			var modelPlaceholder = "留空跟随，下拉选或手填 provider/model";
