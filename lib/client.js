/** dsh-plugin-research 浏览器半（手写 __ModuleLoader__ 工厂，无构建链），v1 设置页形态：
 *  「设置 → Research 调研」独立 section 页（宿主 General 分区左列表新条目，settings.section keyed slot）。
 *
 ...
 *    工具 execute 时读同一份——保存即对下一次调用生效，无需重启 host。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CONTRIBUTION = {
			...
			descriptors: [
				descriptor("getConfig", "_hint", { file: "src/cordis.ts", line: 59, column: 9 }),
				descriptor("setConfig", "patch", { file: "src/cordis.ts", line: 72, column: 9 }),
			],
		};
		...
		function apply(ctx) {
			...
			// 宿主 General 分区左列表的独立 section 页（order 31：usage-stats 30 之后）。
			ctx.effect(function () {
			...
			}, "dsh-plugin-research: settings section");
		}
