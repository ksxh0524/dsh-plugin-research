 ...
 *  DSH 实例 PluginConfigForm 同形折叠卡（STANDARDS §4.1/§4.2：默认折叠成一行，点开才渲染控件；
 ...
 *
 *  形态规范 = STANDARDS §4.1/§4.2（照 DSH 实例 dsh-client-ui-settings-plugins 发布物 PluginConfigForm +
 ...
 *  - 复用面：Tag / chevron 图标走 DSH 实例冻结模块种子表的 `@deepseek-ai/dsh-client-ui-primitives`（§4.3）；
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ResearchCard(props) {
			...
			/** Tag / chevron 优先复用 DSH 实例共享控件（§4.3 种子表），缺席退 CSS 替身。 */
			function tag(text) {
