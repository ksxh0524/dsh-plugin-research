 ...
 *  席位门（STANDARDS §6）：卡注册进 `plugins.item`（id === settings 命名空间），组件按 `{view}`
...
test("复用面与结构件：primitives 走 require，卡壳不退回平铺面板", () => {
  assert.match(CLIENT, /require\("@deepseek-ai\/dsh-client-ui-primitives"\)/, "未复用 DSH 实例 primitives（§4.3）");
  ...
  assert.match(CSS_BLOCK, /\.rsch-card\{[^}]*border-radius:16px/, "卡壳圆角未照 DSH 实例 .card 值（§4.1）");
...
test("席位：plugins.item 注册 + view 双态 + 旧槽零字面量（§6 配对门）", () => {
